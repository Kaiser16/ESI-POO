...
Fecha::Fecha(int d,int m,int a): dia_(d), mes_(m), anno_(a)
{
	...
	{
		time_t calendario = std::time(nullptr);
    	tm* fecha = localtime(&calendario);
...
{
	int parametros;
	parametros = sscanf(cadena_fecha,"%d/%d/%d",&dia_,&mes_,&anno_);
	if(parametros != 3)
		throw Fecha::Invalida("Numero de parametros Incorrecto");
	if(dia_ == 0 || mes_ == 0 || anno_ == 0)
	{
		time_t calendario = std::time(nullptr);
    	tm* fecha = localtime(&calendario);
